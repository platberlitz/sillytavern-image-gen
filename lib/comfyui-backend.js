...
const COMPONENT_FILE = /\.(safetensors|ckpt|pt|pth|bin|gguf|sft|onnx|vae)$/i;
const COMPONENT_INPUT = /^(ckpt|unet|model|vae|clip|clip_[a-z0-9]+|text_encoder|encoder|lora|gguf|diffusion_model)_?(name\d*|file|path)?\d*$/i;
const MAX_COMPONENT_VALUE_BYTES = 4096;
const MAX_COMPONENT_ENTRIES = 256;

/**
 * List every literal filename-like node input in a custom workflow, whatever the node class.
 * Eligibility is by value/input shape, so third-party loaders are covered without an allowlist.
 */
export function discoverComfyWorkflowComponents(workflow) {
    ...
    for (const [nodeId, node] of Object.entries(nodes)) {
        for (const [inputName, rawValue] of Object.entries(node.inputs)) {
            ...
            if (HAS_PLACEHOLDER.test(rawValue)) continue;
            if (!COMPONENT_FILE.test(rawValue) && !COMPONENT_INPUT.test(inputName)) continue;
            ...
            if (found.length >= MAX_COMPONENT_ENTRIES) return found;
        }
    }
    return found;
}
...
export function normalizeComfyWorkflowComponentOverrides(value) {
    ...
    for (const entry of source) {
        ...
        if (!nodeId || !classType || !inputName || !override || override === rawValue) continue;
        if (byteLength(override) > MAX_COMPONENT_VALUE_BYTES) continue;
        const key = `${nodeId}\u0000${classType}\u0000${inputName}`;
        if (seen.has(key)) continue;
        seen.add(key);
        entries.push({ nodeId, classType, inputName, rawValue, value: override });
...
function applyComponentOverrides(renderedNodes, rawWorkflow, overrides) {
    ...
    for (const entry of entries) {
        ...
        if (!isRecord(rawNode) || !isRecord(node)) continue;
        if (rawNode.class_type !== entry.classType) continue;
        if (!hasOwn(rawNode.inputs, entry.inputName)) continue;
        if (rawNode.inputs[entry.inputName] !== entry.rawValue) continue;
